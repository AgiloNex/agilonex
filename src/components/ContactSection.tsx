...
const ContactSection = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmedName || !segmento || !isValidWhatsapp(trimmedWhatsapp) || whatsappDigits.length !== 11) {
      toast.error("Ops, algo deu errado. Tente pelo WhatsApp diretamente.", {
        action: {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Webhook returned a non-2xx response");
      }
        ...
        <div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-[1.25fr_0.85fr] md:gap-8">
          ...
          >
            ...
            <p className="mt-4 max-w-xl text-pretty text-base text-muted-foreground md:text-lg">
              Preencha o formulário e nossa equipe entra em contato em até 1 hora no horário comercial.
            </p>
            <div className="mt-8 grid gap-4 text-sm text-muted-foreground sm:grid-cols-2">
              <div className="rounded-2xl border border-border/70 bg-card/60 p-4 shadow-lg shadow-black/10 backdrop-blur">
                <p className="font-medium text-foreground">Resposta rápida</p>
                <p className="mt-1">Fluxo pensado para captura imediata de leads locais.</p>
              </div>
              <div className="rounded-2xl border border-border/70 bg-card/60 p-4 shadow-lg shadow-black/10 backdrop-blur">
                <p className="font-medium text-foreground">Foco em conversão</p>
                <p className="mt-1">Contato direto pelo WhatsApp e rotina de atendimento comercial.</p>
              </div>
            </div>
          </motion.div>
            ...
            {success ? (
              <div className="flex h-full min-h-[360px] flex-col items-center justify-center rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-8 text-center">
                ...
                <p className="text-xl font-semibold text-foreground">
                  ✅ Recebemos seu contato, {submittedName}! Em breve nossa equipe vai te chamar no WhatsApp.
                </p>
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={segmento} onValueChange={(value) => setSegmento(value as Segmento)}>
                    <SelectTrigger
                      id="segmento"
                      className="h-11 border-border/70 bg-background/90 focus:ring-2 focus:ring-blue-500/80 focus:ring-offset-0"
                    >
          ...
          >
            <h3 className="text-xl font-semibold text-foreground">Contato direto</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Use estes canais se preferir falar com a equipe sem preencher o formulário.
            </p>
